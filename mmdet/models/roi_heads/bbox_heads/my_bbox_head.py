...
class MYBBoxHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None):
        super(MYBBoxHead, self).__init__(init_cfg)
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
                           pos_gt_labels, pos_aug_labels, cfg):
        ...
        # print(pos_gt_labels.shape, pos_aug_labels.shape)
        ...
        aug_labels = pos_bboxes.new_zeros(num_samples)
        bbox_targets = pos_bboxes.new_zeros(num_samples, 4)
        gt_bbox_targets = pos_bboxes.new_zeros(num_samples, 4)
        ...
        if num_pos > 0:
            ...
            aug_labels[:num_pos] = pos_aug_labels
            ...
            gt_bbox_targets[:num_pos, :] = pos_gt_bboxes
            bbox_targets[:num_pos, :] = pos_bbox_targets
            bbox_weights[:num_pos, :] = 1
        if num_neg > 0:
            label_weights[-num_neg:] = 1.0

        return labels, label_weights, bbox_targets, bbox_weights, aug_labels, gt_bbox_targets
    ...
    def get_targets(self,
        ...
        for i, res in enumerate(sampling_results):
            # print(res, gt_aug_labels[i])
            aug_idx = torch.flatten(torch.nonzero(gt_aug_labels[i]))
            bbox_aug_labels = torch.zeros_like(res.pos_assigned_gt_inds, dtype=torch.uint8)
            if len(aug_idx) != 0:
                for idx in iter(aug_idx):
                    pos = torch.where(res.pos_assigned_gt_inds==idx)[0]
                    bbox_aug_labels[pos] = 1  
            pos_aug_labels_list.append(bbox_aug_labels)
        
        labels, label_weights, bbox_targets, bbox_weights, aug_labels, gt_bbox_targets = multi_apply(
        ...
        if concat:
            ...
            aug_labels = torch.cat(aug_labels, 0)
            gt_bbox_targets = torch.cat(gt_bbox_targets, 0)
        return labels, label_weights, bbox_targets, bbox_weights, aug_labels, gt_bbox_targets
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                losses['loss_bbox'] = self.loss_bbox(
                    ...
                    gt_bbox_targets[pos_inds.type(torch.bool)],
                    aug_labels[pos_inds.type(torch.bool)],
    ...
    def refine_bboxes(self, rois, labels, bbox_preds, pos_is_gts, img_metas):
        ...
        Example:
            ...
            >>> self = MYBBoxHead(reg_class_agnostic=True)
            >>> n_roi = 2
            >>> n_img = 4
            >>> scale = 512
